...
try:
    from orchestral_integration import (
        analyze_pr_with_orchestral_agents,
        get_cache_stats,
        clear_cache,
    )
...
try:
    ...
    print(
        f"Warning: Failed to initialize Weave. Observability will be disabled. Error: {e}"
    )
    os.environ["WANDB_MODE"] = "disabled"
...
class Vulnerability(BaseModel):
    ...
    reasoning_path: str = Field(
        description="Chain-of-thought explaining why this is a vulnerability"
    )
...
async def ghostwriter_agent(
    security_report: SecurityReport, validation_report: ValidationReport, pr_info: dict
) -> FinalVerdict:
    """
    ...
    try:
        ...
        return FinalVerdict(
            status="error", comment=f"Failed to synthesize: {e}", confidence_score=0.0
        )
...
async def orchestration_agent(request: AnalysisRequest):
    ...
    final_verdict = await ghostwriter_agent(
        sec_report, run_report, {"repo_id": request.repo_id, "pr_id": request.pr_id}
    )
...
def cache_stats():
    ...
    if not ORCHESTRAL_AVAILABLE:
        raise HTTPException(
            status_code=503, detail="Orchestral agent system not available"
        )
    try:
